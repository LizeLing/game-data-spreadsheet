...
describe('사이드바', () => {
  ...
  describe('템플릿 갤러리', () => {
    ...
    it('"Include sample data" 체크박스를 토글할 수 있어야 함', async () => {
      const user = userEvent.setup();
      ...
      await waitFor(() => {
        expect(
          screen.getByLabelText('샘플 데이터 포함')
        ).toBeInTheDocument();
      });
  ...
  describe('템플릿 시트 생성', () => {
    ...
    it('체크 시 샘플 데이터를 포함해야 함', async () => {
      ...
      const mockTemplate = {
        ...
        name: 'Character Data',
        columns: [{ id: 'id', name: 'ID', type: 'text' as const }],
        sampleData: [{ id: 'char_001' }, { id: 'char_002' }],
      };
    ...
    it('샘플 데이터 체크 해제 시 빈 행을 생성해야 함', async () => {
      const user = userEvent.setup();
      ...
      const checkbox = screen.getByLabelText('샘플 데이터 포함');
      await user.click(checkbox);
